...
object RecipeToDietsTable: IntIdTable("recipe_to_diets") {
    ...
    fun recipeContainTag(recipeId: Int, tagId: Int) = transaction {
        selectAll().where { (RecipeToDietsTable.recipeId eq recipeId) and (dietsId eq tagId) }.count() > 0
    }
    ...
    fun getTag(recipeId: Int) = transaction {
        val categoryId = select(categoryId).where { RecipeToDietsTable.recipeId eq recipeId }.firstOrNull()?.get(categoryId)?.value ?: return@transaction null
